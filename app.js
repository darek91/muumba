...
function resize_canvas() {
    if (window.innerWidth > 150 && window.innerHeight > 150) {
        ...
        for (var i = 0; i < okienka.length; i++) {
            if (!okienka[i].dok) {
                if (okienka[i].blok.x > canvas.width)
                    okienka[i].blok.x = canvas.width - 202;
            }
        }
        odswiezOkienka();
...
function handleFileComplete(event) {
    ...
    if (nazwa.length < 30 && adresPliku.length > 2)
        bg.nName("_" + nazwa);
    stage.update();
    if (event.target.ladowanie)
        event.target.ladowanie.koniec();
    if (event.target.fun)
        event.target.fun();
}
function init() {
